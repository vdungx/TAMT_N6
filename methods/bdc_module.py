...
class BDC(nn.Module):
    ...
    def forward(self, x):
        if self.dr is not None and self.dr != self.input_dim:
            # B,C=x.shape
            # x = x.reshape(B,C,1,1)
            x = self.conv_dr_block(x)       #torch.Size([85, 256, 14, 14])
        x = BDCovpool(x, self.temperature)     #torch.Size([85, 256, 256])
        if self.is_vec:
            x = Triuvec(x)
        else:
...
def BDCovpool(x, t):
    ...
    a1=torch.eye(dim, dim, device=x.device)
    a2=a1.view(1, dim, dim)
    a3=a2.repeat(batchSize, 1, 1)
    I=a3.type(x.dtype)
    ...
    dcov = I_M.bmm(x_pow2 * I) + (x_pow2 * I).bmm(I_M) - 2 * x_pow2  #对角线二倍减二倍整体
    
    dcov = torch.clamp(dcov, min=0.0)  #小于0的设置为0
